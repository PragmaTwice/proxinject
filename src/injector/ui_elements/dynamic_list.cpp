...
void dynamic_list_s::update(basic_context const &ctx) const {
  if (_composer) {
...
bool dynamic_list_s::click(const context &ctx, mouse_button btn) {
  if (!_cells.empty()) {
...
bool dynamic_list_s::text(context const &ctx, text_info info) {
  if (_focus != -1) {
...
void dynamic_list_s::new_focus(context const &ctx, int index) {
  if (_focus != -1) {
...
bool dynamic_list_s::key(const context &ctx, key_info k) {
  ...
  if (_focus != -1) {
    // when tab is pressed at the end of the scroller, it doesn't automatically
    // scroll
    if (try_key(_focus))
      return true;
  }
  ...
  // I commented that since it caused crashes. Still don't know if that can be
  // useful to adapt.
...
bool dynamic_list_s::cursor(const context &ctx, point p,
...
void dynamic_list_s::drag(const context &ctx, mouse_button btn) {
...
bool dynamic_list_s::scroll(context const &ctx, point dir, point p) {
  if (!_cells.empty()) {
    hit_info info = hit_element(ctx, p, true);
    if (auto ptr = info.element;
        ptr && elements::intersects(info.bounds, ctx.view_bounds())) {
...
bool dynamic_list_s::wants_focus() const {
  for (std::size_t ix = 0; ix != _cells.size(); ++ix)
...
bool dynamic_list_s::wants_control() const {
  for (std::size_t ix = 0; ix < _cells.size(); ++ix)
...
void dynamic_list_s::begin_focus() {
  if (_focus == -1)
    _focus = _saved_focus;
  if (_focus == -1) {
    for (std::size_t ix = 0; ix != _cells.size(); ++ix)
      if (_cells[ix].elem_ptr != nullptr &&
          _cells[ix].elem_ptr->wants_focus()) {
        _focus = ix;
        break;
      }
  }
...
element const *dynamic_list_s::focus() const {
  return (_cells.empty() || (_focus == -1)) ? 0 : _cells[_focus].elem_ptr.get();
}

element *dynamic_list_s::focus() {
  return (_cells.empty() || (_focus == -1)) ? 0 : _cells[_focus].elem_ptr.get();
}

void dynamic_list_s::focus(std::size_t index) {
  if (index < _cells.size())
    _focus = int(index);
}

void dynamic_list_s::reset() {
  _focus = -1;
  _saved_focus = -1;
  _click_tracking = -1;
  _cursor_tracking = -1;
...
void dynamic_list_s::resize(size_t n) {
  this->_composer->resize(n);
  this->update();
}

dynamic_list_s::hit_info
dynamic_list_s::hit_element(context const &ctx, point p, bool control) const {
  auto &&test_element = [&](int ix, hit_info &info) -> bool {
    if (_cells[ix].elem_ptr == nullptr) {
      return false;
    }
...
////////////////////////////////////////////////////////////////////////////
// Vertical dynamic_list_s methods
////////////////////////////////////////////////////////////////////////////
float dynamic_list_s::get_main_axis_start(const rect &r) { return r.top; }

float dynamic_list_s::get_main_axis_end(const rect &r) { return r.bottom; }

view_limits
dynamic_list_s::make_limits(float main_axis_size,
                            cell_composer::limits secondary_axis_limits) const {
  return {{secondary_axis_limits.min, float(main_axis_size)},
          {secondary_axis_limits.max, float(main_axis_size)}};
}

void dynamic_list_s::make_bounds(context &ctx, float main_axis_pos,
                                 cell_info &cell) {
  ctx.bounds.top = main_axis_pos + cell.pos;
  ctx.bounds.height(cell.main_axis_size);
}

rect dynamic_list_s::bounds_of(context const &ctx, int ix) const {
  rect r = ctx.bounds;
