...
struct minhook {
	...
	template <typename F>
	static status enable(F* api) {
		return MH_EnableHook(reinterpret_cast<LPVOID>(api));
	}

	template <typename F>
	static status disable(F* api) {
	...
	static status enable() {
		return enable(all_hooks);
	}

	static status disable() {
		return disable(all_hooks);
	}
	...
	template <typename F>
	static status remove(F* target) {
		return MH_RemoveHook(reinterpret_cast<LPVOID>(target));
	}

	template <auto syscall, typename derived>
	struct api {
		...
		static status remove() {
			return minhook::remove(syscall);
		}
	};
};
